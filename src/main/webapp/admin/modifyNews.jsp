...
<script type="text/javascript">
	...
	function submitData(){
		...
		if(title==null || title==''){
			...
			$.post("${pageContext.request.contextPath}/admin/news/save",{'id':'${param.id}','title':title,'newsType.id':newsTypeId,'content':content,'contentNoTag':UE.getEditor('editor').getContentTxt(),'summary':UE.getEditor('editor').getContentTxt().substr(0,155),'keyWord':keyWord},function(result){
				if(result.success){
					alert("新闻修改成功！");
				}else{
					alert("新闻修改失败！");
...
<div id="p" class="easyui-panel" title="修改新闻" style="padding: 10px">
 ...
 <script type="text/javascript">
    ...
    ue.addListener("ready",function(){
        //通过ajax请求数据
        UE.ajax.request("${pageContext.request.contextPath}/admin/news/getById",
            {
                method:"post",
                async : false,  
                data:{"id":"${param.id}"},
                onsuccess:function(result){
                	...
                	$("#keyWord").val(result.keyWord);
       				$("#newsTypeId").combobox("setValue",result.newsType.id);
